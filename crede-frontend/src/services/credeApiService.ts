export async function pollForProof(jobId: string, maxAttempts = 60) { // 10 minutes max by default
...
export async function generateAndWaitForProof(payload: ProofGenerationPayload) {
    try {
      ...
      const response = await fetch("https://crede-api-2-5f9c65727a97.herokuapp.com/generate-proof", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
...
export async function verifyProof(payload: ProofVerificationPayload) {
    try {
      // Initial proof generation request
      const response = await fetch("https://crede-api-2-5f9c65727a97.herokuapp.com/verify-proof", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
